...
# seem like it is not a good habit
# from module1 imoprt *									# 导入module1中的全部变量 使用不必添加module1前缀

# actually I am not clear about this,oops
...
#--	最普通的类
#class C1(C2, C3):									#C2, C3 is the father class
class C1:
	spam = 42														# 数据属性
...
#--	Python的类没有基于参数的函数重载
#seems that the override of the type() function could achieve it
class FirstClass:
...
#--	子类扩展超类: 尽量调用超类的方法
class Person:
	percent=0.1									#this attribute has no actual use
...
#take a little change, take Person and Manager class for example
a=Person(400)
b=Manager(500)
Person.percent=0.2					#default value is 0.1
a.percent							#0.2
b.percent							#0.2
a.percent=0.3						#0.3
Person.percent						#0.2
...
class wrapper:
	def __init__(self, object):
		self.wrapped = object
	#here must be __getattr__, seems like the origin file misses the rear __
...
#-- 获取对象信息: 属性和方法
# this is a useful link telling about the class attr and instance attr
...
setattr(a, "don't touch", 12)				# 这里的属性名带有空格，不能直接访问
getattr(a, "don't touch", 0)					# 但是可以用getattr获取
...
class Methods:
	...
	#take care of the parameters of the classmethod
	#the parameters are the difference between staticmethod and classmethod
...
# decorator: it is like the compound function
# f(x)--> g(f(x))
...
# in python3, the next() should be replaced by __next__()
# http://stackoverflow.com/questions/15150640/line-60-in-make-tuple-return-tuplel-typeerror-iter-returned-non-iterator-o
class Fib:
	def __init__(self):
...
# I skip the instances of the error, you could have a try
...
# # this style is more concise and is very common in operating files
# # 例子:
...
class FormatError(Exception):
	...
	def __init__(self, line ,file):
		...
		self.file = file
	def logger(self):
		open(self.logfile, 'a').write('Error at {}{}'.format(self.file,self.line))
try:
	raise FormatError(42, 'test.py')
except FormatError as X:
	X.logger()
